...
import { Users, Mail, Phone, TrendingUp, Home, UserPlus } from 'lucide-react';
import { useInvitationStats } from '@/hooks/useInvitationStats';
import { useCRMContacts } from '@/hooks/useCRMContacts';

export const RealtorOverview: React.FC = () => {
  ...
  const mortgagePartners = contacts.filter(contact => 
    contact.contact_type === 'professional' && contact.professional_type === 'mortgage'
  );
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Mortgage Partners</CardTitle>
            <Home className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{mortgagePartners.length}</div>
            <p className="text-xs text-muted-foreground">
              Lending partners
            </p>
          </CardContent>
                  ...
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {contact.contact_type === 'client' ? 'Client' : 'Mortgage Professional'}
