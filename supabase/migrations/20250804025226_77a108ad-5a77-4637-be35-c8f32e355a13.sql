-- Final comprehensive cleanup of orphaned records and fix legacy user deletion

-- Clean up all orphaned records before adding CASCADE constraints

-- Remove orphaned client_profiles
DELETE FROM client_profiles 
WHERE professional_id NOT IN (SELECT id FROM professionals);

-- Remove orphaned contact_interactions
DELETE FROM contact_interactions 
WHERE contact_id NOT IN (SELECT id FROM contacts);

-- Remove orphaned client_invitations
DELETE FROM client_invitations 
WHERE professional_id NOT IN (SELECT id FROM professionals);

-- Remove orphaned client_team_assignments
DELETE FROM client_team_assignments 
WHERE professional_id NOT IN (SELECT id FROM professionals);

-- Remove orphaned client_activity_logs
DELETE FROM client_activity_logs 
WHERE professional_id NOT IN (SELECT id FROM professionals);

-- Remove orphaned client_communications
DELETE FROM client_communications 
WHERE professional_id NOT IN (SELECT id FROM professionals);

-- Remove orphaned contacts
DELETE FROM contacts 
WHERE owner_id NOT IN (SELECT id FROM professionals);

-- Remove orphaned clients
DELETE FROM clients 
WHERE professional_id NOT IN (SELECT id FROM professionals);

-- Fix the diagnose_user_data function
