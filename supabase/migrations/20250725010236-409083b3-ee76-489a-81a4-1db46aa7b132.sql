...
-- 1. Add RLS policies to property_cache table
...
CREATE POLICY "Users can update their own cached properties" 
ON public.property_cache 
FOR UPDATE 
USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own cached properties" 
ON public.property_cache 
FOR DELETE 
USING (auth.uid() = user_id);
...
CREATE TRIGGER user_role_change_audit_trigger
