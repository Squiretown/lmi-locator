-- First, let's completely drop any existing delete_user_references functions
DROP FUNCTION IF EXISTS public.delete_user_references(uuid);
DROP FUNCTION IF EXISTS public.delete_user_references(p_target_user_id uuid);
DROP FUNCTION IF EXISTS public.delete_user_references(target_user_id uuid);

-- Now create the function with the standard parameter name that matches RPC expectations
