-- Fix Bug #1: Update contacts table RLS policies to work with professional IDs
-- First, completely drop ALL existing policies on contacts
DO $$ 
DECLARE
    pol record;
BEGIN
...
-- Recreate with proper professional ID checking
CREATE POLICY "Professionals can view their own contacts v2"
ON public.contacts
FOR SELECT
USING (
  owner_id IN (
...
CREATE POLICY "Professionals can insert their own contacts v2"
ON public.contacts
FOR INSERT
WITH CHECK (
  owner_id IN (
...
CREATE POLICY "Professionals can update their own contacts v2"
ON public.contacts
FOR UPDATE
USING (
  owner_id IN (
...
CREATE POLICY "Professionals can delete their own contacts v2"
ON public.contacts
FOR DELETE
USING (
  owner_id IN (
...
-- Fix Bug #2: Update crm_contacts_view to include manual contacts from contacts table
...
-- Manual contacts from contacts table (NEW)
SELECT
  c.id,
  NULL::uuid as user_id,
