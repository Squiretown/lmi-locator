...
BEGIN
  ...
  SELECT 
    au.id,
    CASE 
      WHEN (au.raw_user_meta_data->>'user_type')::text = 'realtor' THEN 'realtor'
      ELSE 'mortgage_professional'
...
BEGIN
  ...
  IF NEW.email_confirmed_at IS NOT NULL AND OLD.email_confirmed_at IS NULL THEN
    INSERT INTO public.professionals (
      ...
      NEW.id,
      CASE 
        WHEN (NEW.raw_user_meta_data->>'user_type')::text = 'realtor' THEN 'realtor'
        ELSE 'mortgage_professional'
      END,
      COALESCE(
        TRIM(CONCAT(
