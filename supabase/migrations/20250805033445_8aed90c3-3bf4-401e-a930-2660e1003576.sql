-- Fix the delete_user_references function to resolve parameter ambiguity
CREATE OR REPLACE FUNCTION public.delete_user_references(p_target_user_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT ARRAY_AGG(p.id) INTO professional_ids FROM professionals p WHERE p.user_id = p_target_user_id;
